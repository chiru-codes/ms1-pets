...
def list_vaccines(
        ...
        pet_id: Optional[int] = Query(None, description="Filtrar vacunas por mascota"),
):
    return vaccine_service.get_vaccines(db, pet_id=pet_id)


@router.get("/{vaccine_id}", response_model=VaccineResponse)
def get_vaccine(vaccine_id: int, db: Session = Depends(get_db_session)):
...
def update_vaccine(vaccine_id: int, vaccine_update: VaccineUpdate, db: Session = Depends(get_db_session)):
...
def delete_vaccine(vaccine_id: int, db: Session = Depends(get_db_session)):
